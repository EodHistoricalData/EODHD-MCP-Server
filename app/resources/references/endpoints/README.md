# Endpoint Index

This folder contains one stub file per endpoint.

## Using `retrieve_description_by_id` for Endpoints

- **Type**: `2` (Endpoints)
- **ID range**: `1` to `84`
- **Get this README**: `type=2, id=0`
- **Get global README**: `type=0`

### Page Index

| ID | Page |
|----|------|
| 1 | Bulk Fundamentals |
| 2 | CBOE Index Data |
| 3 | CBOE Indices List |
| 4 | Company News |
| 5 | Credit CDS Market Aggregates |
| 6 | Credit Corporate CMDI |
| 7 | Credit Corporate HQM Yields |
| 8 | Credit Sovereign CDS Spreads |
| 9 | Credit Sovereign Credit Ratings |
| 10 | Credit Sovereign Default Spreads |
| 11 | Credit Sovereign Risk Premium |
| 12 | Earnings Trends |
| 13 | Economic Events |
| 14 | Exchange Details |
| 15 | Exchange Tickers |
| 16 | Exchanges List |
| 17 | Fundamentals Data |
| 18 | Historical Market Cap |
| 19 | Historical Stock Prices |
| 20 | Index Components |
| 21 | Indices List |
| 22 | Insider Transactions |
| 23 | Intraday Historical Data |
| 24 | Investverte ESG List Companies |
| 25 | Investverte ESG List Countries |
| 26 | Investverte ESG List Sectors |
| 27 | Investverte ESG View Company |
| 28 | Investverte ESG View Country |
| 29 | Investverte ESG View Sector |
| 30 | Live Price Data |
| 31 | Macro Indicator |
| 32 | Marketplace Tick Data |
| 33 | News Word Weights |
| 34 | PRAAMS Bank Balance Sheet By ISIN |
| 35 | PRAAMS Bank Balance Sheet By Ticker |
| 36 | PRAAMS Bank Income Statement By ISIN |
| 37 | PRAAMS Bank Income Statement By Ticker |
| 38 | PRAAMS Bond Analyze By ISIN |
| 39 | PRAAMS Report Bond By ISIN |
| 40 | PRAAMS Report Equity By ISIN |
| 41 | PRAAMS Report Equity By Ticker |
| 42 | PRAAMS Risk Scoring By ISIN |
| 43 | PRAAMS Risk Scoring By Ticker |
| 44 | PRAAMS Smart Investment Screener Bond |
| 45 | PRAAMS Smart Investment Screener Equity |
| 46 | Rates Funding Stress |
| 47 | Rates Policy Rates |
| 48 | Rates Reference Rates |
| 49 | Sanctions Entities |
| 50 | Sanctions Programs |
| 51 | Sanctions Sources |
| 52 | Sanctions Vessels |
| 53 | Sentiment Data |
| 54 | Stock Market Logos |
| 55 | Stock Market Logos SVG |
| 56 | Stock Screener Data |
| 57 | Stocks From Search |
| 58 | Symbol Change History |
| 59 | Technical Indicators |
| 60 | TradingHours List Markets |
| 61 | TradingHours Lookup Markets |
| 62 | TradingHours Market Details |
| 63 | TradingHours Market Status |
| 64 | Upcoming Dividends |
| 65 | Upcoming Earnings |
| 66 | Upcoming IPOs |
| 67 | Upcoming Splits |
| 68 | US Live Extended Quotes |
| 69 | US Options Contracts |
| 70 | US Options EOD |
| 71 | US Options Underlyings |
| 72 | US Tick Data |
| 73 | User Details |
| 74 | UST Bill Rates |
| 75 | UST Long-Term Rates |
| 76 | UST Real Yield Rates |
| 77 | UST Yield Rates |
| 78 | WebSockets Realtime |
| 79 | Real Estate Countries |
| 80 | Real Estate Detailed Prices |
| 81 | Real Estate Detailed Series |
| 82 | Real Estate Selected Prices |
| 83 | Congressional Trades |
| 84 | SEC Filings |

## Endpoints

- US Treasury Bill Rates API (ust-bill-rates.md)
- US Treasury Long-Term Rates API (ust-long-term-rates.md)
- US Treasury Yield Rates API (ust-yield-rates.md)
- US Treasury Real Yield Rates API (ust-real-yield-rates.md)
- WebSockets Real-Time Data API (websockets-realtime.md)
- Cboe Index Data API (cboe-index-data.md)
- Cboe Indices List API (cboe-indices-list.md)
- Company News API (company-news.md)
- Earnings Trends API (earnings-trends.md)
- Economic Events API (economic-events.md)
- Exchange Details API (exchange-details.md)
- Exchanges List API (exchanges-list.md)
- Exchange Tickers API (exchange-tickers.md)
- Bulk Fundamentals API (bulk-fundamentals.md)
- Fundamentals Data API (fundamentals-data.md)
- Historical Market Cap API (historical-market-cap.md)
- Historical Stock Prices API (historical-stock-prices.md)
- Insider Transactions API (insider-transactions.md)
- Intraday Historical Data API (intraday-historical-data.md)
- Live Price Data API (live-price-data.md)
- Macro Indicator API (macro-indicator.md)
- Marketplace Tick Data API (marketplace-tick-data.md)
- Index Components API (index-components.md)
- Indices List API (indices-list.md)
- Investverte ESG List Companies API (investverte-esg-list-companies.md)
- Investverte ESG List Countries API (investverte-esg-list-countries.md)
- Investverte ESG List Sectors API (investverte-esg-list-sectors.md)
- Investverte ESG View Company API (investverte-esg-view-company.md)
- Investverte ESG View Country API (investverte-esg-view-country.md)
- Investverte ESG View Sector API (investverte-esg-view-sector.md)
- Praams Bank Balance Sheet By Isin API (praams-bank-balance-sheet-by-isin.md)
- Praams Bank Balance Sheet By Ticker API (praams-bank-balance-sheet-by-ticker.md)
- Praams Bank Income Statement By Isin API (praams-bank-income-statement-by-isin.md)
- Praams Bank Income Statement By Ticker API (praams-bank-income-statement-by-ticker.md)
- Praams Bond Analyze By Isin API (praams-bond-analyze-by-isin.md)
- Praams Risk Scoring By Isin API (praams-risk-scoring-by-isin.md)
- Praams Risk Scoring By Ticker API (praams-risk-scoring-by-ticker.md)
- Praams Multi-Factor Equity Report By Ticker API (praams-report-equity-by-ticker.md)
- Praams Multi-Factor Equity Report By ISIN API (praams-report-equity-by-isin.md)
- Praams Multi-Factor Bond Report By ISIN API (praams-report-bond-by-isin.md)
- Praams Smart Investment Screener Bond API (praams-smart-investment-screener-bond.md)
- Praams Smart Investment Screener Equity API (praams-smart-investment-screener-equity.md)
- Us Options Contracts API (us-options-contracts.md)
- Us Options Eod API (us-options-eod.md)
- Us Options Underlyings API (us-options-underlyings.md)
- News Word Weights API (news-word-weights.md)
- Sentiment Data API (sentiment-data.md)
- Stock Market Logos API (stock-market-logos.md)
- Stock Market Logos API - SVG Extension (stock-market-logos-svg.md)
- Stock Screener Data API (stock-screener-data.md)
- Stocks From Search API (stocks-from-search.md)
- Symbol Change History API (symbol-change-history.md)
- Technical Indicators API (technical-indicators.md)
- TradingHours List All Markets API (tradinghours-list-markets.md)
- TradingHours Lookup Markets API (tradinghours-lookup-markets.md)
- TradingHours Get Market Details API (tradinghours-market-details.md)
- TradingHours Market Status Details API (tradinghours-market-status.md)
- Upcoming Dividends API (upcoming-dividends.md)
- Upcoming Earnings API (upcoming-earnings.md)
- Upcoming Ipos API (upcoming-ipos.md)
- Upcoming Splits API (upcoming-splits.md)
- User Details API (user-details.md)
- Us Live Extended Quotes API (us-live-extended-quotes.md)
- Us Tick Data API (us-tick-data.md)
- Credit & Sovereign Risk API - CDS Market Aggregates (credit-cds-market-aggregates.md)
- Credit & Sovereign Risk API - Corporate CMDI (credit-corporate-cmdi.md)
- Credit & Sovereign Risk API - Corporate HQM Yields (credit-corporate-hqm-yields.md)
- Credit & Sovereign Risk API - Sovereign CDS Spreads (credit-sovereign-cds-spreads.md)
- Credit & Sovereign Risk API - Sovereign Credit Ratings (credit-sovereign-credit-ratings.md)
- Credit & Sovereign Risk API - Rating-based Default Spreads (credit-sovereign-default-spreads.md)
- Credit & Sovereign Risk API - Sovereign Risk Premium (credit-sovereign-risk-premium.md)
- Interest Rates API - Central-Bank Policy Rates (rates-policy-rates.md)
- Interest Rates API - Reference Rates (rates-reference-rates.md)
- Interest Rates API - Funding-Stress Spreads (rates-funding-stress.md)
- Sanctions Screening API - Entities (sanctions-entities.md)
- Sanctions Screening API - Programs (sanctions-programs.md)
- Sanctions Screening API - Sources (sanctions-sources.md)
- Sanctions Screening API - Vessels (sanctions-vessels.md)
- Real Estate Data API - Countries (real-estate-countries.md)
- Real Estate Data API - Selected Property Prices (real-estate-selected-prices.md)
- Real Estate Data API - Detailed Property Prices (real-estate-detailed-prices.md)
- Real Estate Data API - Detailed Series Catalogue (real-estate-detailed-series.md)
- SEC Filings API (sec-filings.md)
