...
def register(mcp: FastMCP):
    ...
    async def resolve_ticker(
        ...
        """
        Resolve a company name, partial ticker, or ISIN to SYMBOL.EXCHANGE format.

        USE THIS FIRST when a user mentions a company by name instead of a ticker symbol.
        Calls the EODHD Search API and returns the best match as SYMBOL.EXCHANGE.
        If ambiguous (multiple exchanges), returns top 3 matches for user selection.
        ...
        Returns:
            ...
            - alternatives (list): if ambiguous, top 3 matches each with Code, Exchange, Name, Type
        """
